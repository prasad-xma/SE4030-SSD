...
export default function PnDUpdate() {

    const location = useLocation();
    const navigate = useNavigate();
    const { pnd } = location.state || {};
    const [userID, setUserID] = useState(null);

    const [title, setTitle] = useState(pnd?.title || '');
    const [description, setDescription] = useState(pnd?.description || '');
    const [causes, setCauses] = useState(pnd?.causes || '');
    const [solution, setSolution] = useState(pnd?.solution || '');
    const [author, setAuthor] = useState(pnd?.author || '');
    ...
    useEffect(() => {
        ...
        const handleSubmit = async (e) =>  {
            ...
            try {
                const updatedPnd = { title, description, causes, solution, author}
    
                const response = await fetch(`http://localhost:3000/api/researcher/pnd/${pnd._id}`, {
                    method: 'PATCH',
                    body: JSON.stringify(updatedPnd),
                    headers: {
                ...
                setTimeout(() => navigate('/researcher/my-pnd'), 1500);
        ...
        if (!pnd) {
            return (
                <div className="p-8 text-center">
                    <h2 className="text-xl font-semibold text-gray-800">No pnd selected for editing</h2>
 ...
 <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
   ...
   <form onSubmit={handleSubmit} className="space-y-6">
     ...
     <div>
       ...
       <input
         type="text"
         onChange={(e) => setCauses(e.target.value)} value={causes}
         required
         className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border"
       />
     </div>
     ...
     <div>
       ...
       <input
         type="text"
         onChange={(e) => setSolution(e.target.value)} value={solution}
         required
         className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border"
       />
     </div>
